...
методи та атрибути. 
    ...
    Останню концепцію ООП яку ми розглянемо це поліморфізм. Поліморфізм - це один із ключових концептів ООП, який 
дозволяє об'єктам мати різні форми або поведінку, базуючись на їх типах.
    Поліморфізм походить від грецьких слів "polys" (багато) та "morph" (форма). У контексті ООП, це відноситься до 
здатності різних класів використовувати методи з однаковою назвою, але з різною реалізацією. Це дозволяє використовувати 
один інтерфейс для різних типів даних.
...
""" Тут make_sound - це метод, що використовується у кожному класі, але його реалізація різна для Cat та Dog. Це дозволяє 
нам викликати make_sound на екземплярі Animal, не знаючи точно, чи це Cat, Dog, чи інший підклас Animal. Ми створили функцію 
animal_sounds яка приймає список тварин і в принципі не важливо якого вони типу, головне, щоб вони реалізували метод make_sound:

Meow
Woof

    Отже поліморфізм дозволяє обробляти об'єкти різних класів, які є похідними від одного базового класу, через спільний інтерфейс 
(тобто через однакові методи). 
    Таким чином, поліморфізм дозволяє функції animal_sounds взаємодіяти з об'єктами Cat та Dog як з об'єктами Animal, використовуючи 
їхні спільні інтерфейси, не зважаючи на різницю в їх внутрішній реалізації. Це робить код більш гнучким і він легше адаптується до змін, 
оскільки ми можемо додавати нові класи, які наслідують від Animal, без необхідності змінювати функцію animal_sounds.
    З поліморфізмом тісно пов'язано поняття качина типізація.
    
    Качина типізація (Duck Typing) - це концепція в програмуванні, яка відіграє важливу роль в динамічно типізованих мовах, таких як 
    Python. Назва походить від англійського вислову "Якщо це ходить як качка і крякає як качка, то це, ймовірно, качка".

    У контексті програмування, качина типізація означає, що замість перевірки типу об'єкта перед його використанням, важливіше 
зосередитися на тому, чи має об'єкт потрібні методи чи властивості, які вимагаються для виконання певної функції або операції.
    Механізм Python дозволяє використовувати будь-які об'єкти один замість іншого, аби в обох були потрібні методи та поля. 
Інтерпретатор не перевіряє, що у функцію або метод був переданий об'єкт потрібного або дочірнього класу, достатньо щоб в об'єкта 
...
""" У цьому прикладі, функція make_it_quack приймає параметр duck, але фактично не перевіряє, чи це дійсно об'єкт 
класу Duck. Замість цього, вона просто викликає метод quack на переданому об'єкті. Якщо об'єкт має метод quack, він 
може бути використаний функцією make_it_quack незалежно від його фактичного типу. Це і є сутність качиної типізації. 
Головне, щоб атрибут називався так само і приймав ті самі аргументи (якщо це метод).
    В такому підході є переваги так і недоліки. До переваг можна віднести, що код стає більш гнучким, оскільки він 
може працювати з будь-яким об'єктом, що відповідає певному інтерфейсу або поведінці. Перевагою є зниження залежності 
від конкретних типів, що сприяє більш модульній архітектурі.
    Але виникає можливість помилок у виконанні часу, коли передається об'єкт, який не має необхідного методу або 
властивості, що є досить великим недоліком. Це може ускладнити читання та розуміння коду, оскільки не завжди очевидно, 
яких інтерфейсів очікує функція або метод.
    Python є динамічно типізованою мовою, але з версії 3.5, в мову було додано підтримку для анотацій типів. Ці анотації 
типів дозволяють розробникам вказувати очікувані типи для змінних, аргументів функцій та значень, які функції повертають. 
Чим ми і користуємось в конспекті. Але використання таких анотацій типів відбувається на власний розсуд розробника і не 
є обов'язковим.
    Качина типізація зосереджена на поведінці (методах та властивостях) об'єкта, а не на його конкретному типі. Це 
відповідає ідеології "якщо воно веде себе як качка, то це, ймовірно, качка".
    Статична типізація допомагає підвищити читабельність коду, полегшити рефакторинг і зменшити кількість помилок, які 
можуть бути виявлені ще до виконання програми. Вона використовується для вказівки та перевірки типів даних.
    У Python можна використовувати статичну типізацію для анотації типів і одночасно покладатися на качину типізацію 
для поліморфізму та гнучкої поведінки об'єктів.
Розглянемо наступний приклад: """
class Dog:
    def speak(self) -> str:
        return "Woof"
...
# Meow
# Beep boop
""" У цьому прикладі, качина типізація дозволяє нам передавати будь-який об'єкт, який має метод speak, у функцію 
make_it_speak, не зважаючи на його конкретний клас. Але, що стосується типу параметру speaker для функції make_it_speak?
...
які цей параметр має виконувати, не прив'язуючись до конкретного класу.
    Створимо інтерфейс, використовуючи typing.Protocol, для об'єктів, які можуть "говорити". Ми хочемо, щоб будь-який 
об'єкт, який має метод speak, вважався сумісним з цим інтерфейсом. """
...
make_it_speak(cat)  # Виведе: Meow
make_it_speak(robot)  # Виведе: Beep boop
""" Результат буде той самий але статична типізація за допомогою typing.Protocol використовується для вказівки, що 
параметр speaker повинен відповідати інтерфейсу, який має метод speak.
